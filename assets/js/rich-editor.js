...
function toolbarHtml(options={}){const specialKinds=Array.isArray(options.specialKinds)?options.specialKinds:["note","rp","secret","dialogue","toggle"],specialButtons=[["note","＋ メモ"],["rp","＋ RP"],["secret","＋ 秘匿"],["dialogue","💬 会話"],["toggle","▸ トグル"]].filter(([kind])=>specialKinds.includes(kind)).map(([kind,label])=>`<button type="button" data-rich-special="${kind}">${label}</button>`).join(""),insertKinds=Array.isArray(options.insertTools)?options.insertTools:[],insertLabels={toggle:"▸ トグル",image:"🖼 画像",url:"🔗 URL取り込み",conversation:"💬 会話",mediaGroup:"▦ メディア"},insertButtons=insertKinds.map(kind=>`<button type="button" data-rich-insert="${esc(kind)}">${insertLabels[kind]||esc(kind)}</button>`).join("");return `<div class="sm-rich-toolbar sm-rich-ui" contenteditable="false">
